...
function bestFromPacks(packs){ if(!packs||packs.length<5) return null; const legal=f=>f.reduce((a,p)=>a+price(p),0)<=CAP;
  const cands=packs.map(pk=>pk.players.filter(p=>p.capPct!=null)); if(cands.some(c=>!c.length)) return null;
  ...
  let five=[]; for(let i=0;i<cands.length;i++){ const rest=mins.slice(i+1).reduce((a,b)=>a+b,0); let bp=null;
    for(const p of cands[i]){ const spend=five.reduce((a,q)=>a+price(q),0)+price(p); if(spend+rest>CAP) continue; if(!bp||valueOf(p)>valueOf(bp)) bp=p; } if(bp) five.push(bp); } if(five.length<5) return null;
