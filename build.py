import os,json as _json; os.chdir(os.path.dirname(os.path.abspath(__file__)))
...
eng=open('src/engine.js').read(); packs=open('src/packs.js').read(); build=open('src/build.js').read(); shell=open('src/shell.js').read()
...
<script>window.__stage="engine";</script><script>{eng}</script>
...
<script>{packs}</script><script>{build}</script><script>{shell}</script></body></html>'''
